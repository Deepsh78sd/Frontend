...
import { BarChart3, Clipboard, Home, History, FileText, LogOut } from "lucide-react";
...
const ShelterLayout = ({ children }: ShelterLayoutProps) => {
  const location = useLocation();
  
  const navItems = [
    { name: "Dashboard", path: "/shelter/dashboard", icon: <BarChart3 className="w-5 h-5" /> },
    { name: "Applications", path: "/shelter/applications", icon: <Clipboard className="w-5 h-5" /> },
    { name: "Pets", path: "/shelter/pets", icon: <Home className="w-5 h-5" /> },
    { name: "History", path: "/shelter/history", icon: <History className="w-5 h-5" /> },
    { name: "Pet Care Information", path: "/shelter/petcare", icon: <FileText className="w-5 h-5" /> },
  ];
      ...
      <header className="bg-white border-b sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-16">
          <div className="flex items-center">
            <Link to="/shelter/dashboard" className="flex items-center text-teal-500 font-medium">
              ...
              <span className="ml-2 px-2 py-1 bg-indigo-100 text-indigo-800 text-xs font-medium rounded-md">Shelter</span>
            </Link>
          </div>
