...
import { BarChart3, Home, Calendar, FileText, LogOut } from "lucide-react";
...
const HospitalLayout = ({ children }: HospitalLayoutProps) => {
  ...
  const navItems = [
    { name: "Dashboard", path: "/hospital/dashboard", icon: <BarChart3 className="w-5 h-5" /> },
    { name: "Pets", path: "/hospital/pets", icon: <Home className="w-5 h-5" /> },
    { name: "Pet Care Information", path: "/hospital/petcare", icon: <FileText className="w-5 h-5" /> },
      ...
      <header className="bg-white border-b sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-16">
          <div className="flex items-center">
            <Link to="/hospital/dashboard" className="flex items-center text-teal-500 font-medium">
              ...
              <span className="ml-2 px-2 py-1 bg-blue-100 text-blue-800 text-xs font-medium rounded-md">Hospital</span>
            </Link>
          </div>
